...
### [치킨 배달](백준/Gold/18405. 경쟁적 전염) x 2
...
- (bfs)와 재귀를 융합한 재밌는 문제 or 시간을 node 필드 변수로 선언해서 풀 수도 있음. 

- 2번째 풀이
  - 왜이렇게 시간 복잡도 오래 걸렸지?
  - 아 치킨거리 그냥 장애물 없는 맨해튼 거리니까 bfs까지 할 필요 없었음..
  - 최단 거리라고 해서 무조건 bfs인 건 아니다. 장애물이 없다면 맨해튼 거리.
  - 🔥 최단 거리 -> bfs 먼저 떠올리는 건 맞되, 최선의 선택인지 점검은 해보자.
...
### [석유 시추](<../프로그래머스/2/250136. 석유 시추>) — 2026-09-21, 38분

- 덩어리별 크기를 구하고 걸친 열마다 한 번씩 더한 접근은 맞음. result[c]는 c열에서 얻는 총 석유량.
- 🔥 격자 상하좌우는 방향 배열로 처리하자! 연구소 기록에서 썼던 방식 다시 적용하기.
...
- DR[d], DC[d]는 같은 인덱스끼리 한 쌍. 순서대로 상(-1,0), 하(1,0), 좌(0,-1), 우(0,1).
- 🔥 시작점도 큐에 넣을 때 방문 표시! 위 반복문은 시작점 삽입·방문 표시 이후의 이웃 탐색 부분임.
- visited를 꺼낼 때만 표시하면 아직 큐에 있는 칸이 다시 삽입될 수 있음. 2×2 전체 석유로 중복 집계 확인하기.
- 큐는 ArrayDeque. 다음 좌표의 범위 → 석유 여부 → 방문 여부 확인 후 넣기.
- 🔥 시간복잡도는 코드 중첩만 보고 곱하지 말자. 모든 BFS 탐색의 합은 O(NM).
- 다만 덩어리 K개마다 길이 M의 mark 생성·전체 순회 → O(NM + KM), 최악 O(NM²).
- 연결된 덩어리가 걸친 열은 최소 열부터 최대 열까지 연속. minCol~maxCol에만 크기를 더하면 전체 O(NM).
- 각 덩어리의 열 수 ≤ 그 덩어리의 칸 수이므로 구간 갱신을 모두 합쳐도 O(NM).
- Arrays.stream(arr).max().getAsInt(): 빈 배열이면 예외. 이번 result는 길이가 1 이상이므로 안전.
- 🔥 “더 자세히 설계”를 행동으로: 상태 의미, 공유 범위, 방문 표시 시점, 중복 집계 방지, 후처리 비용을 먼저 적기.
- [원본 분석·개선 전체 코드·회고·검증 1,188건](<../프로그래머스/2/250136. 석유 시추/FEEDBACK_2026-09-21.md>)
...
### [타겟 넘버](프로그래머스/2/43165. 타겟 넘버) x 3
- **매개변수와 같은 이름으로 필드에 선언해두면 안 돼!!**
- 규칙 찾아서 이를 통해 풀려고 하니 오히려 더 복잡해짐. depth가 안 정해져 있어서.
- 🔥 규칙 찾는 게 능사는 아님. 다양한 풀이를 고려해야 함.(출제의도파악)

- 두 번째 풀이: 조합으로 풀었음. 
  - 조합에서 다음 메서드에 인자 넘겨줄 때 idx+1이 아니라 i+1이다!!!! 현재 index를 기준으로 해야 해!

- 세 번째 풀이: 전처리x
  - 백트랙킹 본질을 파악하고 풀어라
    - 🔥 for문 안에 dfs 생각해봐. 모든 경우의 수 돌기 위함. 이걸 풀어헤치면 dfs가 여러 줄 나올 수 있음!! 반복문에 얽매이지 말고, 어떤 결과를 구해야하는 지부터 파악하고, 그에 따라 코드 짜자.
...
- 🔥 [2026-09-21 정정] 일반적인 BFS는 시작점을 포함해 큐에 넣을 때 방문 표시함. 이웃의 방문 여부는 넣기 전에 확인함.
- visited는 '이미 발견해서 처리하기로 한 정점'이라는 뜻. while 안인지보다 큐 삽입과 방문 표시의 시점이 중요함.
- 꺼낼 때 표시하는 방식도 가능하지만, 중복 삽입이 생기므로 꺼낸 직후 방문 여부 검사로 중복 처리를 막아야 함.
...
- 세 번째 풀이 - bfs
  ...
  - 하나의 시행을 여러 번 반복할 때 사용한 것들 능동적으로 초기화 한 점 good


### [아이템 줍기](프로그래머스/3/87694. 아이템 줍기) x 2
...
- 🔥 아는 알고리즘이라도 최대한 머리 쓰면서 코드 짜자. 머리 놓고 코드 짜지마!! 분명 실수 나온다.
  - null 에러는 array index랑은 관련없다!! 선언, 할당한 부분을 유의있게 봐라. 
...
### [여행경로]() x3
- 첫 번째 풀이: 전처리x tickets 그대로 이용. visited 이용해서.
...
- 두 번째 풀이: Map<String, List<String>>으로 전처리 후, dfs 사용
  - 첫 번째 풀이보다 시간 복잡도 대체로 좋음. 
  - 🔥 문자열 사전 순 비교: "a".compareTo("b"); // -1
  - 🔥 컬렉션에서 get, poll, peek, pop 등 할 때 우선 원소가 있는지 확인하자!!! (null 에러 발생할 수 있음)
  - 🔥 배열 깊은 복사: arr.clone();
  - 🔥 리스트 깊은 복사: ArrayList<Integer> deepCopyList = new ArrayList<>(originalList);
  - 🔥 한 줄 한 줄 생각 담아서 해야해. 생각 없이 편한대로 이거겠지 하고 해버리면 나중에 디버깅 할 때 고생한다...!

- 세 번째 풀이: 두 번째 풀이랑 비슷함. 단, 달라진 건 정렬!
...
### [불량 사용자](프로그래머스/3/64064. 불량 사용자) x2
...
- 백트랙킹 if에서 sort하면 영향미치니까 하려면 clone!! 이거 잘 실천함.
- set '배열 중복검사' 효율 올리는 법? - 문자열로 join 하는 거 나쁘지 않았음.
...
### [가장 먼 노드](../프로그래머스/3/49189. 가장 먼 노드) x2
- 🔥 그래프 종류에 따른 최단 거리 접근
...
### [미로 탈출]
- 작은 기능, 확실한 것부터 잘 구현했다!
- 그래프 전처리 빡세게 할 필요없었음. 각 좌표 따로 저장하니까, 지나갈 수 있는 길인지 없는지만 판단했으면 됨.
- 완벽하게 하려 하지 않고 우선 구현에만 집중한 점 굿.


### [물통]
- 와우 기존 dfs, bfs와 다른 신선한 문제
- 기존에는 인접 리스트, 인접 행렬을 기준으로 dfs, bfs 문제 냄새가 폴폴 났다면, 이번에는 그래프 탐색인 걸 알아채기 어려웠음.
  - 물통의 상태 자체를 그래프의 노드로 인식함.
  - visited를 3차원 배열로 표현함. 
  - bfs는 최단 거리에서만 쓰이는 게 아님. 
  - 🔥 특정 행동을 계속 반복해야 한다면 -> 재귀 -> 백트랙킹, dfs, bfs
  - 🔥 그래프 탐색의 핵심 원리 - 반복, 노드, visited(종료)
    - 특정 시행을 계속해서 반복한다.
    - 하나의 노드를 무엇으로 잡든지간에 일관성이 존재하면 된다.
    - visited, depth 등을 통해서 종료 조건을 만족한다.
- 이 문제를 풀려면
  - 🔥 결과적 사고보다는 순차적 사고로 설계했어야 함.
  - 🔥 하나의 시행 단위를 인식했어야 함. 
    - 그리고 하나의 시행에서 두 가지 경우의 수가 나올 수 있음을 파악해야 함.
    - 하나의 시행이 여러 번 반복된다는 걸 파악해야 함.
  - visited를 센스있게 다룰 줄 알았어야 함.
- 배열 다른 함수로 넘겨서 처리할 때 조심. 원하지 않은 변경이 일어날 수 있다!!


### [맥주 마시면서 걸어가기]
- dfs 풀이
  - visited 복원을 하지 않아도 되는 이유?
    - 🆘 복원을 하지 않는다는 건 방문 순서가 필요없다는 것.
    - 최단 경로가 아니기에 무엇을 먼저 방문하는 지는 중요하지 않음.
    - 하나의 편의점을 방문했다면, 갈 수 있는 인근 편의점을 모두 방문하게 됨. 그리고 이게 모든 편의점에 대해서 계속해서 적용됨.
      - 그렇게 해서 페스티벌에 갔다면 해피고, 못 갔다면 그 편의점은 더 이상 갈 필요가 없음.
      - 그렇기에 순서 바꿔서 편의점을 방문할 이유가 없음. 그저 시작점에서 방문할 수 있는 모든 편의점에 대해 dfs 걸어두면 됨.
    - 🆘 하나의 시행단위를 정확히 인식했다면?
    - 🆘 '백트랙킹에서의 본질 - 순서, 위아래' 순서가 필요한지 점검했다면?
    - 🆘 떠올린 알고리즘이 있을 때, 해당 알고리즘이 적용되는지 엄밀히 검증했다면? (전형적인 상황이 아니니)
    - 순서가 왜 안 필요한지 더 깊게 느껴보기
  - clone() 남발하면 메모리 초과 일으킬 수도 있다!! 특히 반복문에서.. 불필요한 클론x

- bfs, 플로이드 워셜 풀어보기 


### [토마토]
- bfs 여러 번 하면 되는 간단한 유형이었음. 
  - 그나마 색다른 건 시간 비용 처리. 근데 이것도 인지하고만 있으면 처리하기 쉬움.
  - 근데 큐 선입선출이니까, 마지막에 꺼낸 time을 답으로 하면 됨...!