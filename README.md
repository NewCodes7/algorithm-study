...
- 2번 빙산 46분 런타임 에러
  ...
  - 큐를 꼭 썼어야 했나?
    - 시간 복잡도 추정과 실제 실행 시간은 표본에 따라 다를 수 있음.
...
class Solution {
    private static class Prog implements Comparable<Prog> {
        int rank;
        int wait;
        int amount;
        
        public Prog(int a, int b, int c) {
            this.rank = a;
            this.wait = b;
            this.amount = c;
        }
        ...
        @Override
        public int compareTo(Prog other) {
            return this.rank - other.rank;
        }
    }
    
    public long[] solution(int[][] program) { // 점수, 호출시각, 실행 시간
        long[] answer = new long[11];
        
        Queue<int[]> q = new LinkedList<>();
        Arrays.sort(program, (a, b) -> a[1] - b[1]); // 호출 시각 기준 오름차순
        for (int[] c : program) q.offer(c);
        
        PriorityQueue<Prog> q2 = new PriorityQueue<>(); // 우선순위 정해야 해..!!
        
        int time = 0;
        int curr = 0;
        while (!(curr == 0 && q.isEmpty() && q2.isEmpty())) { // 여기 반대로!! 실수 ✅
            // 현재 실행 중인게 끝났다면
            if (curr == 0) {
                // 대기 큐에 있던 거 실행, 대기 시간만큼 ++하기
                if (!q2.isEmpty()) {
                    Prog c = q2.poll();
                    answer[c.rank] += c.wait;
                    curr = c.amount;
                }
                // 대기 큐에 없다면 time++하면서 기다리기 - 대기 큐 삽입에서 해결됨.
            }
            // System.out.println("p" + q.peek()[1]); // 아.. 요놈 때문에 ㅡㅡ✅
            
            // 대기 큐 삽입
            while (!q.isEmpty() && q.peek()[1] == time) { // 기억!✅
                if (!q.isEmpty()) {
                    int[] c = q.poll();
                    
                     // 현재 실행중인 게 있다면 대기 큐에 삽입
                    if (curr != 0) {
                        c[1] = 0; // !! 호출된 시각은 이제 필요없으니 대기 시간으로 활용하기
                        q2.offer(new Prog(c[0], c[1], c[2])); 
                    } else { // q에서 바로 실행
                        curr = c[2];
                    }
                }
            ...
            // 제2큐에서 대기중인 것들 ++
            if (!q2.isEmpty()) { //✅
                for (Prog c : q2) { // !! 이거 되나? 아... q가 아니라 q2!!!!
                    c.wait++;
                }
            }
        }
...
/*
3:48~
점수 낮을수록, 우선순위 높음. (점수 == 랭킹)

우선순위 큐? -> 이게 좋을 듯.
 호출된 시각에 따라, 점수에 따라

처음 배열 - 호출 시각을 기준으로 정렬 -> 큐

1초 단위로 반복 - 큐가 빌때까지 && 지금 실행하고 있는 게 끝날 때까지
    지금 실행하고 있는 것 --
    poll해서 호출 시각과 같은 것들 모조리 꺼내기 (제 2의 큐 - 우선 순위를 기준으로 정렬)
    대기중인 것들 ++
    시간++
    if (지금 실행 == 0) 
        제 2의 큐에서 하나 꺼내기
        제 2의 큐에서 묵힌 시간만큼 대기 시간 더하기 
...
설계를 다시해야 할 듯 너무 복잡함.
순차적으로 구현한 아이디어를 다시 다듬고 정리하는 과정 있어야 함. 
큐 하나로 풀 수도 있을 듯.
